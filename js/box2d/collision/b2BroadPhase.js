...
function b2BroadPhase(worldAABB, callback){
  ...
  for (i = b2Settings.b2_maxProxies; i--;){
  ...
  for (i = 0; i < 2; i++){
    ...
    for (var j = 2*b2Settings.b2_maxProxies; j--;){
  ...
  var tProxy;
  for (i = b2Settings.b2_maxProxies - 1; i--;)
  {
    tProxy = new b2Proxy();
...
b2BroadPhase.prototype = {
	...
	DestroyProxy: function(proxyId){
		...
		for (var i = this.m_queryResultCount; i--;)
		{
			//b2Settings.b2Assert(this.m_proxyPool[this.m_queryResults[i]].IsValid());
	...
	QueryAABB: function(aabb, userData, maxCount){
		...
		for (var count = 0; count < this.m_queryResultCount && count < maxCount; ++count)
		{
			//b2Settings.b2Assert(this.m_queryResults[i] < b2Settings.b2_maxProxies);
			var proxy = this.m_proxyPool[ this.m_queryResults[count] ];
			//b2Settings.b2Assert(proxy.IsValid());
			userData[count] = proxy.userData;
		}
...
//private:
	...
	IncrementTimeStamp: function(){
		...
		{
			for (var i = b2Settings.b2_maxProxies; i--;)
			{
