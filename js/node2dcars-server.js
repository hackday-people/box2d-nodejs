...
//			drawShape(s, context, b);
    ...
    if (outputarr.length) server.broadcast(JSON.stringify({bodies:outputarr}));
//    drawWorld(world, ctx);
    missedFrames = 0;
    frames += 1;
...
//    jQuery('#delayVal').text((delayAvg / frames).toFixed(1));
  ...
  setTimeout(step, 10+((delay > 0) ? delay : 0));
}
...
server.addListener("connection", function(conn){
    ...
    conn.addListener("message", function(message){
        if(message[0] == "/"){
            ...
            try {
//                var inputs = JSON.parse(message);
            } catch (e) {
//                 sys.error("bad json: "+message+e); 
            }
            if (inputs) {
//                moveCar(conn.id, inputs.f, inputs.b, inputs.l, inputs.r)
            }
            server.broadcast(conn.storage.get("username")+": "+message+server.manager.length);
        }
    });
});
